...
def change_room(direction):
    ...
    global boss_intro_shown_this_entry, shop_items, room_acquire_type, weapons
...
#                     info["target_pos"] = info["start_pos"]
    ...
    for enemy in enemies:
        ...
        if dist_sq < r_sum * r_sum:
            ...
            if dash_active:
                # 대쉬 중: 플레이어는 멈추지 않도록, 적을 한 번만 밀어낸다
                key = id(enemy)
                if key not in enemies_pushed_this_frame:
                    push_x = (dx / dist) * penetration
                    push_y = (dy / dist) * penetration
                    enemy.world_x += push_x
                    enemy.world_y += push_y
                    enemies_pushed_this_frame.add(key)
                # 플레이어 측 관통 누적은 생략 → 대쉬가 끊기지 않게 함
            else:
